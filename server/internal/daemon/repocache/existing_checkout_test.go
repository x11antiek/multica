...
func taskBranchName(taskID string) string {
	return "agent-runs/agent/" + taskKey(taskID)
}

func localAgentBranches(t *testing.T, repoPath string) string {
	t.Helper()
	out, err := runGitOutput("-C", repoPath, "for-each-ref", "--format=%(refname)", "refs/heads/agent-runs/")
	if err != nil {
...
func TestIsTaskBranch(t *testing.T) {
	t.Parallel()
	const branch = "agent-runs/agent/111111111111"
	for _, tc := range []struct {
		current string
		want    bool
	}{
		...
		{current: "agent-runs/agent/222222222222", want: false},
		{current: "", want: false},
	} {
