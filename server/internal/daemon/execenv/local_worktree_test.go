...
func TestPrepareLocalWorktreeReplaysUncommittedWork(t *testing.T) {
	...
	if want := "agent-runs/j/" + taskKey("11112222-3333-4444-5555-666677778888"); wt.Branch != want {
...
func TestPrepareLocalWorktreeFailsWhenUntrackedReplayIsTruncated(t *testing.T) {
	...
	if branches := gitRun(t, repo, "branch", "--list", "agent-runs/*"); branches != "" {
...
func TestPrepareLocalWorktreeContinuesTheConversationBranch(t *testing.T) {
	...
	if first.Branch != "agent-runs/j/mul-6881" {
		t.Fatalf("first turn branch = %q, want agent-runs/j/mul-6881", first.Branch)
	}
	if first.Continued {
	...
	if firstOutcome.Branch != "agent-runs/j/mul-6881" {
		t.Fatalf("first outcome branch = %q", firstOutcome.Branch)
	}
	firstTip := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881")

	second := prepareTurn(t, repo, "MUL-6881", turnTwoTask)
	if second.Branch != "agent-runs/j/mul-6881" {
	...
	// those with it.
	if outcome := finalizeOK(t, third); outcome.Branch != "agent-runs/j/mul-6881" {
	...
	for file, want := range map[string]string{"turn-one.txt": "work from turn one", "turn-two.txt": "work from turn two"} {
		if got := gitRun(t, repo, "show", "agent-runs/j/mul-6881:"+file); got != want {
	...
	branches := gitRun(t, repo, "branch", "--list", "agent-runs/j/*")
	if strings.Count(branches, "agent-runs/j/") != 1 {
...
func TestPrepareLocalWorktreeHandsConflictingUserEditsToTheAgent(t *testing.T) {
	...
	finalizeOK(t, first)
	recordedAfterFirst := gitRun(t, repo, "rev-parse", userStateRef("agent-runs/j/mul-6881"))
	...
	if got := gitRun(t, repo, "rev-parse", userStateRef("agent-runs/j/mul-6881")); got != recordedAfterFirst {
		t.Error("the snapshot advanced past an edit the branch never took")
	}
	if got := gitRun(t, repo, "show", "agent-runs/j/mul-6881:tracked.txt"); got != "rewritten by the agent" {
...
func TestConflictResolvedByTheAgentIsDeliveredAndNotReplayedAgain(t *testing.T) {
	...
	if outcome.Branch != "agent-runs/j/mul-6881" {
		t.Fatalf("resolved turn delivered %q", outcome.Branch)
	}
	if got := gitRun(t, repo, "show", "agent-runs/j/mul-6881:tracked.txt"); got != "B and C" {
...
func TestDiscardKeepsTheContinuedBranch(t *testing.T) {
	...
	if _, err := gitTry(t, repo, "rev-parse", "--verify", "agent-runs/j/mul-6881"); err != nil {
...
func TestPrepareLocalWorktreeForksWhenTheConversationBranchIsBusy(t *testing.T) {
	...
	tip := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881")
	...
	if want := "agent-runs/j/mul-6881-" + taskKey(turnTwoTask); sibling.Branch != want {
...
func TestPrepareLocalWorktreeRestartsAMergedConversationBranch(t *testing.T) {
	...
	gitRun(t, repo, "merge", "--no-edit", "agent-runs/j/mul-6881")
...
// because the name matches. `agent-runs/j/mul-6881` is a name the user can type
...
func TestPrepareLocalWorktreeRefusesToAdoptABranchItDoesNotOwn(t *testing.T) {
	...
	gitRun(t, repo, "branch", "agent-runs/j/mul-6881")
	gitRun(t, repo, "worktree", "add", "--quiet", filepath.Join(t.TempDir(), "theirs"), "agent-runs/j/mul-6881")
	userBranchDir := ""
	...
	userTip := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881")
	...
	if want := "agent-runs/j/mul-6881-" + testBranchOwner.fingerprint(); first.Branch != want {
	...
	if got := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881"); got != userTip {
...
func TestPrepareLocalWorktreePrunesSnapshotsOfDeletedBranches(t *testing.T) {
	...
	writeFile(t, filepath.Join(first.WorkDir, "agent.txt"), "turn one\n")
	finalizeOK(t, first)
	ref := userStateRef("agent-runs/j/mul-6881")
	...
	gitRun(t, repo, "branch", "-D", "agent-runs/j/mul-6881")
...
func TestBranchRecordRoundTrips(t *testing.T) {
	...
	recorded, err := writeBranchRecord(repo, "agent-runs/j/mul-6881", snapshot, head, testBranchOwner)
	if err != nil {
...
func TestPrepareLocalWorktreeRefusesABranchDeletedAndRecreatedUnderTheSameName(t *testing.T) {
	...
	gitRun(t, repo, "branch", "agent-runs/j/mul-6881")
	theirs := t.TempDir()
	gitRun(t, repo, "worktree", "add", "--quiet", filepath.Join(theirs, "wt"), "agent-runs/j/mul-6881")
...
func TestPrepareLocalWorktreeRefusesABranchForceMovedOffItsRecord(t *testing.T) {
	...
	writeFile(t, filepath.Join(first.WorkDir, "agent.txt"), "turn one\n")
	finalizeOK(t, first)
	delivered := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881")
	ref, err := readUserStateRef(repo, "agent-runs/j/mul-6881")
	if err != nil {
	...
	gitRun(t, repo, "branch", "-f", "agent-runs/j/mul-6881", "theirs")
	...
	written, err := writeBranchRecord(repo, "agent-runs/j/mul-6881", record.state, delivered, testBranchOwner)
	if err != nil {
		t.Fatalf("writeBranchRecord: %v", err)
	}
...
func TestFinalizeFailsWhenTheBranchRecordCannotBeWritten(t *testing.T) {
	...
	lock := filepath.Join(repo, ".git", filepath.FromSlash(userStateRef("agent-runs/j/mul-6881"))+".lock")
	...
	if got := gitRun(t, repo, "show", "agent-runs/j/mul-6881:agent.txt"); got != "turn one" {
	...
	gitRun(t, repo, "worktree", "add", "--quiet", worktree, "agent-runs/j/mul-6881")
	...
	second := prepareTurn(t, repo, "MUL-6881", turnTwoTask)
	if second.Continued || second.Branch == "agent-runs/j/mul-6881" {
...
func TestFinalizeRefusesToRecordADeliveryThatResetPastItsBaseline(t *testing.T) {
	...
	gitRun(t, repo, "worktree", "add", "--quiet", theirs, "agent-runs/j/mul-6881")
...
func TestFinalizeRefusesToRecordADeliveryFromOffTheBranch(t *testing.T) {
	...
	if got := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881"); got != delivered {
...
func TestConflictAfterAUserCommitOnTheBranchStillOffersTheEditAgain(t *testing.T) {
	...
	gitRun(t, repo, "worktree", "remove", "--force", worktree)
	movedTip := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881")
	...
	gitRun(t, second.Path, "add", "tracked.txt")
	if outcome := finalizeOK(t, second); outcome.Branch != "agent-runs/j/mul-6881" {
		t.Fatalf("resolved turn delivered %q", outcome.Branch)
	}
	if got := gitRun(t, repo, "rev-parse", "agent-runs/j/mul-6881"); got != movedTip {
...
func TestIsolatedPrepareCarriesTheStateFinalizeNeeds(t *testing.T) {
	...
	if wt.Branch != "agent-runs/j/mul-6881" {
	...
	if !second.LocalWorktree.Continued || second.LocalWorktree.Branch != "agent-runs/j/mul-6881" {
