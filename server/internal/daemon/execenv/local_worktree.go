...
type LocalWorktreeParams struct {
	...
	// it gets: agent-runs/<name>/<short-task-id>.
	AgentName string
	TaskID    string
...
// the user recognises in `git branch`, agent-runs/j/mul-6881 rather than a uuid
...
func (p taskBranchPlan) altName(taskID string) string {
	if p.conversational {
		...
		// from: agent-runs/j/mul-6881-<task>.
...
// `agent-runs/j/mul-6881` is a name the user can type, another agent of the same
...
// `agent-runs/j/mul-6881-<fingerprint>` — stable for this exact conversation, so its
...
func resolveTaskBranch(gitRoot string, params LocalWorktreeParams, headSHA string, logger *slog.Logger) taskBranchPlan {
	...
	taskScoped := taskBranchPlan{name: fmt.Sprintf("agent-runs/%s/%s", agentSegment, taskKey(params.TaskID)), base: headSHA}

	owner := params.owner()
	if params.ConversationKey == "" || !owner.valid() {
		return taskScoped
	}
	preferred := fmt.Sprintf("agent-runs/%s/%s", agentSegment, sanitizeName(params.ConversationKey))
