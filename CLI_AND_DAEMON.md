...
| GC Codex session TTL (native rollouts) | — | `MULTICA_GC_CODEX_SESSION_TTL` | `0` (disabled; set a duration to opt in) |
| GC Hermes session TTL (per-conversation `state.db`) | — | `MULTICA_GC_HERMES_SESSION_TTL` | `0` (disabled; set a duration to opt in) |
...
- **Completed-task retention bound** — `MULTICA_GC_COMPLETED_TASK_TTL` fully removes an inactive issue task once its `.gc_meta.json` `completed_at` age exceeds the configured duration, even while the parent issue remains open. Cleanup waits for a successful parent-issue status check, never removes an active environment, and never fully removes a `local_directory` environment. A later rerun provisions a fresh environment instead of resuming the removed checkout.
  ...
  - Removing an environment discards work an agent left uncommitted or unpushed on its branch, along with that task's `output/` and `logs/`. The scoped Codex session store lives outside `MULTICA_WORKSPACES_ROOT`, so a later rerun still resumes the agent's prior session — it just starts from a fresh checkout. Transcript pruning is disabled by default; if you opt in with `MULTICA_GC_CODEX_SESSION_TTL`, size it against that trade and keep it comfortably above `MULTICA_GC_INTERVAL`: the active-root guard protects a task that is currently running, not one whose follow-up run is queued but unclaimed.
...
- **Provider transcript retention** — Multica-owned native transcript stores live outside `MULTICA_WORKSPACES_ROOT`: Codex rollouts under `~/.codex/multica-sessions/`, Hermes `state.db` files under `<profile dir>/hermes-sessions/`, and managed-MCP Cursor data under `<profile dir>/provider-sessions/cursor/`. Issue and chat tasks share a conversation key; one-shot and autopilot tasks receive their own durable key. Claude Code, OpenCode, Antigravity and unmanaged Cursor already write to their provider-level user data directories because the daemon does not redirect `HOME`, so task GC cannot remove them. Native stores retain the provider's exact bytes, including provider-specific tool and subagent events; normalized Multica task messages are not treated as a replacement. Transcript pruning is opt-in: `MULTICA_GC_CODEX_SESSION_TTL` and `MULTICA_GC_HERMES_SESSION_TTL` default to `0`. When an operator supplies a positive TTL, a store idle past it is removed; a store held by a running task is never reclaimed. Transcript stores can contain prompts, tool output and secrets and should remain readable only by the daemon owner.
